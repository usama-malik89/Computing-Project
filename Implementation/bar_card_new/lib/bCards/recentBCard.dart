...
//Class for showing a variation of the Card in the recent section of the app
class RecentBCard extends StatefulWidget {
  ...
  _RecentBCardState createState() => _RecentBCardState();
}

class _RecentBCardState extends State<RecentBCard> {
  ...
  //function to show a popup dialog to confirm the user wants to remove the card from recent scans
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text("Do you want to remove this card from your history?"),
              ],
            ),
          ),
          actions: <Widget>[ //list that contains the two action buttons
            ...
            FlatButton(
              child: Text('Yes'),
              onPressed: () {
                //if yes is pressed it deletes the card from the cardHistory collection in the database
                ...
                //once it has been deleted from the database it refreshes the recent cards section using the parentAction function
    ...
    return FutureBuilder<List<String>>(
      ...
      builder: (BuildContext context, AsyncSnapshot<List<String>> snapshot) {
        ...
        if (snapshot.hasData) {
              ...
              children: <Widget>[
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: stringToColor(snapshot.data[2]),
                  ),
                  padding: EdgeInsets.all(16),
                  child: cardBack(stringToColor(snapshot.data[2]),
                      stringToColor(snapshot.data[3]), snapshot.data[1]),
                ),
                Container(
                  margin: EdgeInsets.symmetric(
                    horizontal: 5,
          ...
          children = LoadingBCard(margin: widget.margin);
        }
